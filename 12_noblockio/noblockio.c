...
文件名		: noblockio.c
描述		: RK3588 GPIO按键非阻塞IO实验，支持3个按键
***************************************************************/
...
static int key_parse_one(struct device_node *child, int index)
{
	...
	if (!gpio_is_valid(data->gpio)) {
		printk("noblockio: can't get key%d gpios\r\n", index);
		return -EINVAL;
	}
	...
	if (ret) {
		printk("noblockio: request %s gpio failed\r\n", data->label);
		return ret;
	}

	ret = gpio_direction_input(data->gpio);
	if (ret) {
		printk("noblockio: set %s input failed\r\n", data->label);
		goto free_gpio;
	}
	...
	if (data->irq_num < 0) {
		ret = data->irq_num;
		printk("noblockio: get %s irq failed\r\n", data->label);
		goto free_gpio;
	}
	...
	ret = request_irq(data->irq_num, key_interrupt, irq_flags,
			  data->label, data);
	if (ret) {
		printk("noblockio: request %s irq failed\r\n", data->label);
		goto free_gpio;
	}

	printk("noblockio: %s gpio=%d irq=%d active_low=%d\r\n",
...
static int key_parse_dt(void)
{
	...
	if (key.nd == NULL) {
		printk("noblockio: lhb-keys node not found!\r\n");
		return -EINVAL;
	}

	if (!of_device_is_available(key.nd)) {
		printk("noblockio: lhb-keys node is disabled!\r\n");
		return -EINVAL;
	}

	if (!of_device_is_compatible(key.nd, "lhb,gpio-keys")) {
		printk("noblockio: compatible match failed\r\n");
		return -EINVAL;
	}
