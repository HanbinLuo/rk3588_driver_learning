...
static int key_parse_one(struct device_node *child, int index)
{
	...
	if (!gpio_is_valid(data->gpio)) {
		printk("blockio: can't get key%d gpios\r\n", index);
		return -EINVAL;
	}
	...
	if (ret) {
		printk("blockio: request %s gpio failed\r\n", data->label);
		return ret;
	}

	ret = gpio_direction_input(data->gpio);
	if (ret) {
		printk("blockio: set %s input failed\r\n", data->label);
		goto free_gpio;
	}
	...
	if (data->irq_num < 0) {
		ret = data->irq_num;
		printk("blockio: get %s irq failed\r\n", data->label);
		goto free_gpio;
	}
	...
	ret = request_irq(data->irq_num, key_interrupt, irq_flags,
			  data->label, data);
	if (ret) {
		printk("blockio: request %s irq failed\r\n", data->label);
		goto free_gpio;
	}

	printk("blockio: %s gpio=%d irq=%d active_low=%d\r\n",
...
static int key_parse_dt(void)
{
	...
	if (key.nd == NULL) {
		printk("blockio: lhb-keys node not found!\r\n");
		return -EINVAL;
	}

	if (!of_device_is_available(key.nd)) {
		printk("blockio: lhb-keys node is disabled!\r\n");
		return -EINVAL;
	}

	if (!of_device_is_compatible(key.nd, "lhb,gpio-keys")) {
		printk("blockio: compatible match failed\r\n");
		return -EINVAL;
	}
