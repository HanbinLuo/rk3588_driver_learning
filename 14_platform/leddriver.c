...
文件名		: leddriver.c
描述		: RK3588 platform LED驱动，支持设备树中的4个GPIO LED
***************************************************************/
...
static int led_parse_one(struct device_node *child, int index)
{
	...
	if (!gpio_is_valid(data->gpio)) {
		printk("platled: can't get led%d gpios\r\n", index);
		return -EINVAL;
	}
	...
	if (ret) {
		printk("platled: request %s gpio failed\r\n", data->label);
		return ret;
	}

	ret = gpio_direction_output(data->gpio, data->active_low ? 1 : 0);
	if (ret) {
		printk("platled: set %s output failed\r\n", data->label);
		gpio_free(data->gpio);
		return ret;
	}

	printk("platled: %s id=%d gpio=%d active_low=%d\r\n",
...
static int led_parse_dt(void)
{
	...
	leddev.node = of_find_node_by_path("/gpio-leds");
	if (leddev.node == NULL) {
		printk("platled: gpio-leds node not found!\r\n");
		return -EINVAL;
	}

	if (!of_device_is_available(leddev.node)) {
		printk("platled: gpio-leds node is disabled!\r\n");
		return -EINVAL;
	}

	if (!of_device_is_compatible(leddev.node, "lhb,gpio-leds")) {
		printk("platled: compatible match failed\r\n");
		return -EINVAL;
	}

	for_each_available_child_of_node(leddev.node, child) {
		if (i >= LED_NUM)
			break;
...
static int led_probe(struct platform_device *pdev)
{
	...
	printk("platled: platform driver and device matched\r\n");
